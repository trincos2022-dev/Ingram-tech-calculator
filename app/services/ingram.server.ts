...
// Track in-flight OAuth token requests to prevent race conditions
...
async function ensureAccessToken(
  ...
  // Check if there's already a token request in flight for this shop
  const requestKey = credentials.shopDomain;
  const inflight = inflightTokenRequests.get(requestKey);
  if (inflight) {
    console.log(
      `[${credentials.shopDomain}] Waiting for in-flight token request`,
    );
    return inflight;
  }
  ...
  inflightTokenRequests.set(requestKey, tokenPromise);
...
export async function syncCarriersFromResponse(
  ...
  console.log(`[syncCarriers] Starting sync for ${shopDomain} with ${distributions.length} distributions`);
  ...
  for (const dist of distributions) {
    ...
    console.log(`[syncCarriers] Processing distribution with ${carriers.length} carriers`);
    ...
    for (const carrier of carriers) {
      ...
      if (!seenCarriers.has(code)) {
        ...
        console.log(`[syncCarriers] Found new carrier: ${code} (${carrier.shipVia || 'no name'})`);
      }
    }
  }

  console.log(`[syncCarriers] Found ${seenCarriers.size} unique carriers to sync`);
  ...
  console.log(`[syncCarriers] Existing carrier configs in DB: ${existingConfigs.length}`);
  ...
  for (const [carrierCode, data] of seenCarriers) {
    ...
    try {
      ...
      if (existing) {
        ...
        newCount++;
        console.log(`[syncCarriers] Created new carrier: ${carrierCode} - ${data.carrierName}`);
      }
    } catch (error) {
      console.error(`[syncCarriers] Failed to upsert carrier ${carrierCode}:`, error);
      throw error; // Re-throw to catch in the calling code
    }
  }

  console.log(`[syncCarriers] Sync complete: ${newCount} new, ${updatedCount} updated, ${seenCarriers.size} total`);
