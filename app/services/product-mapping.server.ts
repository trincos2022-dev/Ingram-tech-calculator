...
export async function getVendor(skus: string[]) {
  if (!Array.isArray(skus) || skus.length === 0) {
    throw new Error("Invalid SKU array");
  }

  const cleanSkus = skus.map(sku => String(sku).trim());

  const supabase = getSupabaseClient();

  const { data, error } = await supabase
    .from("final_product_table_us")
    .select("price_vendor_part, price_part_nbr, source_type")
    .in("price_vendor_part", cleanSkus); // ✅ array filter
