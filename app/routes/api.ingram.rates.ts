...
// Helper to get fallback rate response for carrier service (Ingram-specific)
async function getFallbackRateResponse(
  shopDomain: string,
  currency: string = "USD"
): Promise<{ rates: Array<ReturnType<typeof formatFallbackRateForShopify>> }> {
  const settings = await getFallbackRateSettings(shopDomain, "INGRAM");
    ...
    if (carrierShop) {
      ...
      if (response.response?.errors) {
        ...
        saveCarrierRateLog({
          ...
          vendorRawResponse: response.response,
          durationMs: Date.now() - startTime,
        });
      ...
      if (combinedRates.length === 0) {
        ...
        saveCarrierRateLog({
          ...
          shipToCountry: payload.shipToAddress.countryCode,
          status: "no_rates",
          carriersCount: distributions.length,
          ratesReturned: 0,
