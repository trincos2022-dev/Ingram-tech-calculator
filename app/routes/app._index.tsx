...
type TdRateRequestLogEntry = {
  id: string;
  correlationId: string;
  requestType: string;
  cartItemCount: number;
  cartSkus: string;
  tdPartNums: string | null;
  shipToCity: string | null;
  shipToState: string | null;
  shipToZip: string | null;
  shipToCountry: string | null;
...
type LoaderData = {
  ...
  tdCarrierService?: CarrierServiceSummary | null;
  unifiedCarrierService?: CarrierServiceSummary | null;
  carrierConfigs: CarrierConfig[];
  tdcarrierConfigs: CarrierConfig[];
  rateLogs: RateRequestLogEntry[];
  tdRateLogs: TdRateRequestLogEntry[];
...
const tdCarrierCallbackPath = "/api/techdata/rates";
const unifiedCarrierCallbackPath = "/api/unified/rates";

function getAppCallbackUrl(path: string = carrierCallbackPath) {
...
async function findExistingCarrierService(
  ...
  const callbackUrl = getAppCallbackUrl(callbackPath);
...
async function registerCarrierServiceForShop(
  ...
  const callbackUrl = getAppCallbackUrl(options?.callbackPath);
  const serviceName = options?.name || "Ingram Freight";
  const response = await fetch(
    `https://${shop}/admin/api/${apiVersion}/carrier_services.json`,
    {
      method: "POST",
...
type ActionResult =
  ...
  | {
      ok: true;
      intent: "registerUnifiedCarrierService";
      message: string;
    }
  | {
      ok: true;
      intent: "deleteTdCarrierService";
      message: string;
    }
  | {
      ok: true;
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  let tdCarrierService: CarrierServiceSummary | null = null;
  let unifiedCarrierService: CarrierServiceSummary | null = null;
  try {
    const [ingramService, techDataService, unifiedService] = await Promise.all([
      ...
      findExistingCarrierService(
        session.shop,
        requireAccessToken(session.accessToken),
        tdCarrierCallbackPath,
      ),
      findExistingCarrierService(
        session.shop,
        requireAccessToken(session.accessToken),
        unifiedCarrierCallbackPath,
      ),
    ]);
    carrierService = ingramService;
    tdCarrierService = techDataService;
    unifiedCarrierService = unifiedService;
  } catch (error) {
  ...
  const tdcarrierConfigs = await getTd_CarrierConfigurations(session.shop);

  // Fetch recent rate request logs (last 50)
  let serializedLogs: RateRequestLogEntry[] = [];
  let serializedTdLogs: TdRateRequestLogEntry[] = [];
  try {
    const [rateLogs, tdRateLogs] = await Promise.all([
      prisma.carrierRateRequestLog.findMany({
    ...
    serializedTdLogs = tdRateLogs.map((log) => ({
      ...log,
      createdAt: log.createdAt.toISOString(),
    }));
  } catch (error) {
  ...
  return {
    ...
    carrierService,
    tdCarrierService,
    unifiedCarrierService,
    carrierConfigs,
    tdcarrierConfigs,
    rateLogs: serializedLogs,
    tdRateLogs: serializedTdLogs,
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    switch (intent) {
      ...
      case "registerUnifiedCarrierService": {
        await registerCarrierServiceForShop(
          session.shop,
          requireAccessToken(session.accessToken),
          {
            name: "Unified Freight (Ingram + TechData)",
            callbackPath: unifiedCarrierCallbackPath,
          },
        );
        return {
          ok: true,
          intent,
          message:
            "Unified carrier service registered. Shopify will now call the app for rates from both Ingram and TechData.",
        } satisfies ActionResult;
      }
      case "deleteCarrierService": {
        ...
        return {
          ok: true,
          intent,
          message: "Ingram carrier service removed.",
        } satisfies ActionResult;
      }
      case "deleteTdCarrierService": {
        const id = Number(formData.get("tdCarrierServiceId"));
        if (!id) {
          return {
            ok: false,
            intent,
            message: "TechData carrier service ID missing.",
          } satisfies ActionResult;
        }
        ...
        return {
          ok: true,
          intent,
          message: "TechData carrier service removed.",
        } satisfies ActionResult;
      }
      case "saveCarrierConfig": {
        ...
        return {
          ok: true,
          intent,
          message: `Updated ${updates.length} Ingramcarrier configurations.`,
        } satisfies ActionResult;
      }
      case "saveTd_CarrierConfig": {
        // Get all carrier codes from form data
        const tdcarrierConfigs = await getTd_CarrierConfigurations(
          session.shop,
        );
        const updates: Array<{ carrierCode: string; enabled: boolean }> = [];

        for (const config of tdcarrierConfigs) {
          const isEnabled =
            formData.get(`tdcarrier_${config.carrierCode}`) === "on";
          updates.push({
            carrierCode: config.carrierCode,
            enabled: isEnabled,
          });
        ...
        await updateTd_CarrierEnabledStatus(session.shop, updates);

        return {
          ok: true,
          intent,
          message: `Updated ${updates.length} Techdatacarrier configurations.`,
        } satisfies ActionResult;
      }
      case "saveFallbackRate": {
        ...
        await saveFallbackRateSettings(
          session.shop,
          {
            enabled,
...
export default function Index() {
  ...
  const isRegisteringTdCarrier =
    navigation.formData?.get("_action") === "registerTdCarrierService";
  const isDeletingTdCarrier =
    navigation.formData?.get("_action") === "deleteTdCarrierService";
  const isRegisteringUnifiedCarrier =
    navigation.formData?.get("_action") === "registerUnifiedCarrierService";
  const isDeletingUnifiedCarrier =
    navigation.formData?.get("_action") === "deleteUnifiedCarrierService";
  const isSavingCarrierConfig =
    navigation.formData?.get("_action") === "saveCarrierConfig";
  const isSavingTdCarrierConfig =
    navigation.formData?.get("_action") === "saveTd_CarrierConfig";
    ...
    <s-page heading="Ingram Micro freight rates | TD_Synnex freight quote ">
      ...
      {selectedApp === "ingrammicro" ? (
        <s-stack direction="block" gap="base">
          ...
          <s-section heading="Unified Checkout Carrier Service">
            <s-paragraph>
              This unified carrier service calls both Ingram and TechData APIs
              in parallel, automatically merging rates by carrier code. When a
              customer reaches checkout, Shopify will get rates from both
              vendors in a single request.
            </s-paragraph>
            ...
            {unifiedCarrierService ? (
              <s-stack direction="block" gap="base">
                <s-text>
                  Status:{" "}
                  {unifiedCarrierService.active ? "Active ✓" : "Inactive"}
                </s-text>
                <s-text>Callback: {unifiedCarrierService.callbackUrl}</s-text>
                <s-text>Name: {unifiedCarrierService.name}</s-text>
                <Form method="post">
                  <input
                    type="hidden"
                    name="_action"
                  ...
                  <input
                    ...
                    value={unifiedCarrierService.id}
                  />
                  <s-button
                    tone="critical"
                    {...(isDeletingUnifiedCarrier ? { loading: true } : {})}
                  >
                    Remove unified carrier service
                  </s-button>
                </Form>
              </s-stack>
            ) : (
              <s-stack direction="block" gap="base">
                <Form method="post">
                  <input
                    type="hidden"
                    name="_action"
                    value="registerUnifiedCarrierService"
                  />
                  <s-button
                    type="submit"
                    {...(isRegisteringUnifiedCarrier ? { loading: true } : {})}
                    disabled={!credentials || !tdcredentials}
                  >
                    Register unified carrier service
                  </s-button>
                </Form>
                {(!credentials || !tdcredentials) && (
                  <s-text style={{ marginTop: "0.5rem", color: "#6b7280" }}>
                    Both Ingram and TechData credentials are required to
                    register the unified service.
                  </s-text>
                )}
              </s-stack>
            )}
        ...
        <s-stack direction="block" gap="base">
          <s-section heading="Unified Carrier Service">
            <s-paragraph>
              Use the unified carrier service in the Ingram Micro tab to get
              rates from both Ingram and TechData in a single request. The
              unified service automatically calls both APIs in parallel and
              merges the results.
            </s-paragraph>
            <s-box
              padding="base"
            ...
            >
              <s-text>
                ℹ️ Go to the Ingram Micro tab above to register the unified
                carrier service that handles both vendors.
              </s-text>
            </s-box>
          </s-section>
          ...
          <s-section heading="Shipping method configuration">
            ...
            {tdcarrierConfigs.length > 0 ? (
              <Form method="post">
                <input
                  type="hidden"
                  name="_action"
                  value="saveTd_CarrierConfig"
                />
                <s-stack direction="block" gap="base">
                  <div
                    style={{
                  ...
                  >
                    {tdcarrierConfigs.map((config) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          name={`tdcarrier_${config.carrierCode}`}
                          defaultChecked={config.enabled}
                        />
                        <span>
                          <strong>{config.carrierName}</strong>
                  ...
                  <s-button
                    type="submit"
                    {...(isSavingTdCarrierConfig ? { loading: true } : {})}
                  >
                    Save carrier configuration
                  </s-button>
                </s-stack>
          ...
          <s-section heading="Rate request history">
            <s-paragraph>
              Recent checkout and cart rate requests for TechData. Use this to
              troubleshoot issues with shipping rates not appearing at checkout.
            </s-paragraph>
            ...
            {tdRateLogs.length > 0 ? (
                            ...
                            <tr key={`${log.id}-details`}>
                              ...
                              >
                                ...
                                >
                                  ...
                                  {log.tdPartNums && (
                                    <div>
                                      <strong>TechData Part Numbers:</strong>
                                      <br />
                                      <code style={{ fontSize: "0.75rem" }}>
                                        {log.tdPartNums}
                                      </code>
                                  ...
                                  {log.tdRawResponse && (
                                    <div style={{ gridColumn: "1 / -1" }}>
                                      <strong>TechData Raw Response:</strong>
                                      <pre
                                        style={{
                                          margin: "0.5rem 0 0 0",
                                          padding: "0.5rem",
              ...
              >
                <s-text>
                  No rate requests logged yet. Logs will appear here after
                  checkout requests are made with products that have TechData
                  mappings.
                </s-text>
              </s-box>
            )}
